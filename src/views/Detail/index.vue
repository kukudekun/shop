  ...
  methods: {
    ...
    changeSkuNum (event) {
      ...
      if (isNaN(value) || value < 1) {
        ...
        // 只保留整数parseInt,向上取整Math.ceil 向下Math.floor 四舍五入 Math.round
        this.skuNum = parseInt(value)
      }
    },
    async addShopCar () {
      // 1.发请求，将产品数据加入数据库
      // 2.服务器存储成功进行路由跳转并传递参数
      try {
        await this.$store.dispatch('addOrUpdateShopCart', { skuId: this.$route.params.skuid, skuNum: this.skuNum })
        // 路由跳转，并将所需要的产品信息带到下一页
        // 路由传参将一些简单数据如skuNum通过query形式传递
        // 复杂数据通过会话储存传递（会话本地储存的是字符串）
        // JSON.parse将字符串转对象 JSON.stringify将对象转字
